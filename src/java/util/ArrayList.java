 ...
 *      ensureCapacity(int minCapacity)：若minCapacity>默认容量，判断是否需要扩容，若minCapacity>当前数组内存空间，就需要扩容,扩容按
 *      当前数组内存空间1.5倍进行扩容，扩容之后选择minCapacity和扩容后容量较大者为a，然后再判断是否内存溢出，判断a>Integer最大值-8，则最终容量
 *      取值Integer最大值，否则取值Integer最大值-8
 ...
 * 对于add(int, e)或remove(int)的单个调用,向此字段添加的数量不得超过 1，否则迭代器（和列表迭代器）将抛出虚假的 concurrentmodificationexceptions。
    ...
    private boolean batchRemove(Collection<?> c, boolean complement) {
        ...
        try {
            ...
            for (; r < size; r++)
                ...
                //lxf  若保留，则将相同元素移动到前面；若删除，则将不同的元素移动到前面
